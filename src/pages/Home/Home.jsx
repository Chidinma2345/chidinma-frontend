...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // The fallback community updates below will be displayed
        // when there are no posts in the backend yet.
        setPosts(Array.isArray(res.data) ? res.data.slice(0, 3) : []);
      } catch (err) {
        console.error('Error fetching posts:', err);

        // If the backend is unavailable, the page will still
        // display the community impact content below.
        setPosts([]);
      } finally {
        setLoading(false);
      }
   ...
   *
   * They are intentionally written from the information supplied:
   * - Helping people in the community
   * - Empowering people
   * - Gathering and feeding women
   * - Showing kindness and support to people
   *
   * You can later replace these with verified dated reports,
   * photographs, project details and measurable figures through
   * the News & Updates/admin system.
   */
    ...
    <Box className="home-root">
        ...
        <Container maxWidth="lg">
          ...
          {/* <Box className="impact-note">
            <Typography variant="body2">
              Community activities and updates will continue to be added
              as official reports, photographs, events and announcements
              become available.
            </Typography>
          </Box> */}
      ...
      >
        ...
        {loading ? (
          ...
          /*
           * IMPORTANT:
           * The old "No campaign updates..." empty message has
           * been removed.
           *
           * Instead, the Community Impact section above remains
           * visible so the Home page never looks empty while the
           * backend has no published posts.
           */
